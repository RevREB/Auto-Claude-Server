...
export function UsageIndicator() {
  ...
  useEffect(() => {
    // Listen for real-time usage updates via WebSocket
    const unsubscribe = window.api.onUsageUpdated((snapshot: ClaudeUsageSnapshot) => {
      console.log('[UsageIndicator] Received usage update:', snapshot);
      setUsage(snapshot);
      setIsVisible(true);
    });

    // Request initial usage on mount (backend collects every 60s automatically)
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <Tooltip>
        ...
        <TooltipContent side="bottom" className="text-xs w-64 z-[9999]">
