...
interface GitHubIntegrationProps {
  ...
  projectPath?: string; // Project path for fetching git branches (deprecated)
  ...
  const fetchBranches = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (!envConfig?.defaultBranch && (projectId || projectPath)) {
          debugLog('fetchBranches: No defaultBranch set, auto-detecting...');
          const detectResult = await window.api.detectMainBranch(projectId || projectPath);
