...
export type AppSection = 'appearance' | 'projects' | 'agent' | 'paths' | 'integrations' | 'updates' | 'notifications';
...
const appNavItems: NavItem<AppSection>[] = [
  ...
  { id: 'paths', label: 'Paths', icon: Settings2, description: 'Python and framework paths' },
...
export function AppSettingsDialog({ open, onOpenChange, initialSection, initialProjectSection, onRerunWizard }: AppSettingsDialogProps) {
  ...
  const handleProjectSelectFromSettings = (projectId: string) => {
    // When a project is selected from the Projects page, switch to project settings
    setActiveTopLevel('project');
  ...
  const renderAppSection = () => {
    switch (appSection) {
      ...
      case 'projects':
        return <ProjectsSettings onProjectSelect={handleProjectSelectFromSettings} />;
      case 'agent':
