...
export function GitSetupModal({
  ...
  const handleInitializeGit = async () => {
    ...
    try {
      ...
      const result = await window.api.initializeGit(project.id);
  ...
  const handleSkip = async () => {
    // Tell backend to remember that git setup was skipped for this project
    if (project) {
      try {
        await window.api.skipGitSetup(project.id);
      } catch (err) {
        console.error('Failed to save git setup skip preference:', err);
      }
    }
    onSkip?.();
    onOpenChange(false);
