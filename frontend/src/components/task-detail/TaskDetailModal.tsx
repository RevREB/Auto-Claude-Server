...
function TaskDetailModalContent({ open, task, onOpenChange }: { open: boolean; task: Task; onOpenChange: (open: boolean) => void }) {
  ...
  const handleMerge = async () => {
    console.log('[TaskDetailModal] handleMerge called, stageOnly:', state.stageOnly);
    state.setIsMerging(true);
    state.setWorkspaceError(null);
    try {
      const result = await workspaceApi.merge(task.id, { noCommit: state.stageOnly });
      console.log('[TaskDetailModal] merge result:', result);
      if (result.success) {
        if (state.stageOnly && result.staged) {
          state.setWorkspaceError(null);
          state.setStagedSuccess(result.message || 'Changes staged in main project');
          state.setStagedProjectPath(result.projectPath);
          state.setSuggestedCommitMessage(result.suggestedCommitMessage);
        } else {
          // Task status update comes via WebSocket broadcast from backend
          onOpenChange(false);
        }
      } else {
        state.setWorkspaceError(result.message || 'Failed to merge changes');
      }
    } catch (error) {
      console.error('[TaskDetailModal] handleMerge exception:', error);
  ...
  const handleDiscard = async () => {
    ...
    state.setWorkspaceError(null);
    try {
      const result = await workspaceApi.discard(task.id);
      console.log('[TaskDetailModal] discard result:', result);
      if (result.success) {
        state.setShowDiscardDialog(false);
        onOpenChange(false);
      } else {
        state.setWorkspaceError(result.message || 'Failed to discard changes');
      }
    } catch (error) {
      console.error('[TaskDetailModal] handleDiscard exception:', error);
      state.setWorkspaceError(error instanceof Error ? error.message : 'Failed to discard changes');
    } finally {
