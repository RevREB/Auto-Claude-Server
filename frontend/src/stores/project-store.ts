...
export async function initializeProject(
  projectId: string
): Promise<{ success: boolean; error?: string; data?: { message: string; path: string } }> {
  const store = useProjectStore.getState();
  ...
  try {
    ...
    console.log('[ProjectStore] Initialization failed:', errorMsg);
    store.setError(errorMsg || 'Failed to initialize project');
    return { success: false, error: errorMsg || 'Failed to initialize project' };
  } catch (error) {
...
export async function updateProjectAutoBuild(
  ...
  try {
    const result = await window.api.updateProjectAutoBuild(projectId);
    if (result.success && result.data) {
      return result.data;
    }
