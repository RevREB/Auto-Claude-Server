...
export function setupIdeationListeners(): () => void {
  ...
  const unsubProgress = window.api.onIdeationProgress((_projectId, status) => {
    // Debug logging
    if (window.DEBUG) {
  ...
  const unsubLog = window.api.onIdeationLog((_projectId, log) => {
    store().addLog(log);
  });
  ...
  const unsubComplete = window.api.onIdeationComplete((_projectId, session) => {
    // Debug logging
    if (window.DEBUG) {
  ...
  // Listen for errors
  const unsubError = window.api.onIdeationError((_projectId, error) => {
    // Debug logging
    if (window.DEBUG) {
  ...
  const unsubStopped = window.api.onIdeationStopped((_projectId) => {
    store().setGenerationStatus({
      phase: 'idle',
      progress: 0,
      message: 'Generation stopped'
