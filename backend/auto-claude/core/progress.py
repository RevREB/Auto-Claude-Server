...
def validate_plan_integrity(spec_dir: Path) -> tuple[bool, list[str]]:
    """
    Validate the integrity of implementation_plan.json.

    Checks JSON schema structure and required fields to ensure
    the plan can be processed correctly.
    ...
    # Check file exists
    if not plan_file.exists():
        return False, ["implementation_plan.json not found"]
    ...
    phases = plan.get("phases")
    if phases is None:
        # Already reported as missing field
        return False, errors
    ...
    for i, phase in enumerate(phases):
        ...
        # Phase must have an identifier (id or phase)
        if "id" not in phase and "phase" not in phase:
            errors.append(f"Phase {i} missing identifier (id or phase)")

        # Phase should have a name
        ...
        subtasks = phase.get("subtasks")
        if subtasks is None:
            errors.append(f"Phase {i} missing 'subtasks' field")
            continue

        if not isinstance(subtasks, list):
            errors.append(f"Phase {i} 'subtasks' must be an array")
            continue

        # Validate each subtask
        phase_id = phase.get("id") or phase.get("phase") or f"phase-{i}"
        for j, subtask in enumerate(subtasks):
            if not isinstance(subtask, dict):
                errors.append(f"Subtask {j} in {phase_id} must be an object")
                continue

            # Required subtask fields
            if "id" not in subtask:
                errors.append(f"Subtask {j} in {phase_id} missing 'id' field")

            if "description" not in subtask:
                errors.append(f"Subtask {j} in {phase_id} missing 'description' field")

            if "status" not in subtask:
                errors.append(f"Subtask {j} in {phase_id} missing 'status' field")
            else:
                ...
                if status not in valid_statuses:
                    errors.append(
                        f"Subtask {subtask.get('id', j)} in {phase_id} has invalid status: {status}"
                    )
...
def cleanup_orphaned_tasks(spec_dir: Path) -> int:
    """
    Reset stale in_progress tasks to pending.

    This handles recovery when a session crashes or is interrupted
    while a task was being worked on. Tasks left in 'in_progress'
    state are reset to 'pending' so they can be retried.
        ...
        for phase in plan.get("phases", []):
            for subtask in phase.get("subtasks", []):
                if subtask.get("status") == "in_progress":
                    ...
                    # Clear stale tracking data to ensure clean state
                    subtask.pop("started_at", None)
                    subtask.pop("session_id", None)
                    cleaned_count += 1
...
def recover_from_restart(spec_dir: Path) -> dict:
    """
    Orchestrate recovery from a restart by validating state and cleaning up.

    This function should be called on startup to ensure the implementation
    plan is in a valid state and any orphaned tasks from a previous
    interrupted session are cleaned up.

    The recovery process:
    1. Validates the implementation_plan.json structure and schema
    2. Resets any stale 'in_progress' tasks to 'pending' for retry
    3. Returns a summary of recovery actions taken
    ...
    Returns:
        ...
        - success: True if recovery succeeded, False on validation failure
        - valid: True if plan passed integrity validation
        - errors: List of validation errors (if any)
        - orphaned_tasks_cleaned: Number of stale tasks reset to pending
        - message: Human-readable summary of recovery actions
    """
    result = {
        "success": False,
        "valid": False,
    ...
    if not plan_file.exists():
        result["success"] = True
        result["message"] = "No implementation plan found - nothing to recover"
        return result

    # Step 1: Validate plan integrity
    is_valid, errors = validate_plan_integrity(spec_dir)
    result["valid"] = is_valid
    result["errors"] = errors

    if not is_valid:
        result["success"] = False
        result["message"] = f"Plan validation failed: {'; '.join(errors)}"
        return result

    # Step 2: Clean up orphaned tasks (stale in_progress -> pending)
    cleaned_count = cleanup_orphaned_tasks(spec_dir)
    result["orphaned_tasks_cleaned"] = cleaned_count

    # Build success message
    result["success"] = True
    if cleaned_count > 0:
        result["message"] = (
            f"Recovery complete: reset {cleaned_count} orphaned task(s) to pending"
        )
    else:
        result["message"] = "Recovery complete: plan valid, no orphaned tasks found"
