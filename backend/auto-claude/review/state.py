...
class ReviewState:
    ...
    def save(self, spec_dir: Path) -> None:
        """Save state to database (primary) and file (fallback)."""
        ...
        if HAS_DB_STORAGE:
            try:
                ...
                storage.save_review_state(self.to_dict())
                return  # Success
            except Exception:
    ...
    def load(cls, spec_dir: Path) -> "ReviewState":
        """
        Load state from database or spec directory.

        Returns a new empty ReviewState if not found or invalid.
        """
        spec_id = Path(spec_dir).name
        ...
        if HAS_DB_STORAGE:
            try:
                storage = get_spec_storage(spec_id)
                db_data = storage.load_review_state()
                if db_data:
                    return cls.from_dict(db_data)
            except Exception:
                pass
