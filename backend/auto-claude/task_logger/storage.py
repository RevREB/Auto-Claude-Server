...
class LogStorage:
    ...
    def __init__(self, spec_dir: Path):
        ...
        self._storage = get_spec_storage(self.spec_id) if HAS_DB_STORAGE else None
    ...
    def _load_or_create(self) -> dict:
        ...
        if self._storage:
            try:
                db_data = self._storage.load_task_logs()
                if db_data:
                    return db_data
            except Exception:
                pass
    ...
    def save(self) -> None:
        """Save logs to database (primary) and file (fallback)."""
        self._data["updated_at"] = self._timestamp()

        # Save to database
        if self._storage:
            try:
                self._storage.save_task_logs(self._data)
                return  # Success - don't need file fallback
            except Exception as e:
                print(f"Warning: DB save failed, falling back to file: {e}", file=sys.stderr)
...
def load_task_logs(spec_dir: Path) -> dict | None:
    """
    Load task logs from database or spec directory.
    ...
    if HAS_DB_STORAGE:
        try:
            ...
            db_data = storage.load_task_logs()
            if db_data:
                return db_data
        except Exception:
            pass
